...
# synthesizes them through ONE resident daemon (model loaded once), and joins them.
#
...
#
#   MAXCHARS=800 ./fb2-to-wav.sh book.fb2 ~/piper-models/ru_RU-irina-medium.onnx out.m4b
set -euo pipefail
...
echo "→ $N chunks (~${MAXCHARS} chars each); synthesizing on one resident daemon…"
